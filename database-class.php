...
class DB{
	...
	public $db_host=MOBILEPAYMENT_DB_HOST;
	public $db_name=MOBILEPAYMENT_DB_NAME;
	public $db_user=MOBILEPAYMENT_DB_USER;
	private $db_password=MOBILEPAYMENT_DB_PASSWORD;
    protected $db;
	
	function __construct($db_host=MOBILEPAYMENT_DB_HOST,$db_user=MOBILEPAYMENT_DB_USER,$db_password=MOBILEPAYMENT_DB_PASSWORD,$db_name=MOBILEPAYMENT_DB_NAME){
		try{
	...
	*/
	public function send_email($template_name, $to, $subject, $template_data)
	{
		...
		$template_file_full_path = MOBILEPAYMENT_EMAIL_TEMPLATE_PATH . $template_name;
		...
		$headers  = "From: ".MOBILEPAYMENT_EMAIL_FOR_EMAILHEADER . " MOBILEPAYMENT\r\n";
	...
	*/
	...
	public function remember_me(){
		...
		$UID   = base64_decode($_COOKIE['MOBILEPAYMENT_UID']);
		$UTYPE = base64_decode($_COOKIE['MOBILEPAYMENT_UTYPE']);
		
		if($UTYPE == 1 || $UTYPE == 2 || $UTYPE == 0){
			
			
			$query = "SELECT user_id ,user_email,user_password,user_type,user_owner_id,user_status,user_company_id FROM users WHERE user_id =:user_id AND user_type =:user_type" ;
			$param = array("user_id"=>$UID,'user_type'=>$UTYPE);
			$get_data = self::selectQuery($query,$param);	
			
			$_SESSION['MOBILEPAYMENT_USERID'] = $get_data[0]['user_id'];//set user-id in session.
			$_SESSION['MOBILEPAYMENT_USERTYPE'] = $get_data[0]['user_type'];//set user-type in session.
			$_SESSION['MOBILEPAYMENT_USEROWNERID'] = $get_data[0]['user_owner_id'];//set user-type in session.
			
			if($get_data[0]['user_type']== 1){
				$_SESSION['MOBILEPAYMENT_USERCOMPANYID'] = $get_data[0]['user_id'];
			}else{
				$_SESSION['MOBILEPAYMENT_USERCOMPANYID'] = $get_data[0]['user_company_id'];
			}
			
			
		}elseif($UTYPE == 4 || $UTYPE == 5 || $UTYPE == 3){
			
			$query = "SELECT acc_id ,acc_email,acc_password,acc_type,acc_owner_id,acc_status,acc_company_id FROM accounts WHERE acc_id =:acc_id AND acc_type =:acc_type" ;	
			$param = array("acc_id"=>$UID,"acc_type"=>$UTYPE);
			$get_data = self::selectQuery($query,$param);
			
			$_SESSION['MOBILEPAYMENT_USERID'] = $get_data[0]['acc_id'];//set user-id in session.
			$_SESSION['MOBILEPAYMENT_USERTYPE'] = $get_data[0]['acc_type'];//set user-type in session.
			
			if($get_data[0]['acc_type'] == 4){
				
				$accQ = "SELECT * FROM accounts WHERE acc_users_id LIKE :acc_users_id";
				$accP = array('acc_users_id'=>'%"'.$get_data[0]['acc_id'].'"%');
				$acc  = self::selectQuery($accQ,$accP);
				$_SESSION['MOBILEPAYMENT_USEROWNERID'] = $acc[0]['acc_id'];
				$_SESSION['MOBILEPAYMENT_USERCOMPANYID'] = $acc[0]['acc_company_id'];
				$_SESSION['MOBILEPAYMENT_USERCOMPANYIDADMIN'] = $acc[0]['acc_owner_id'];
				$isadminData = self::get_account_meta($get_data[0]['acc_id'],'account_user_is_admin');
				
				if(!empty($isadminData)){
					$_SESSION['MOBILEPAYMENT_USERISADMIN'] = $isadminData[0]['account_meta_value'];
				}else{
					$_SESSION['MOBILEPAYMENT_USERISADMIN'] = 0;
				}
			}else{
				$_SESSION['MOBILEPAYMENT_USERISADMIN'] = 1;
				$_SESSION['MOBILEPAYMENT_USEROWNERID'] = $get_data[0]['acc_owner_id'];//set user-type in session.
				$_SESSION['MOBILEPAYMENT_USERCOMPANYID'] = $get_data[0]['acc_company_id'];
			}
		}
	}
